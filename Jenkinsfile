pipeline {
  ...
  stages {
    ...
    stage('code promotion') {
      steps {
        sh "cp /var/www/rectangles/all/rectangle_${env.MAJOR_VERSION}.${env.BUILD_NUMBER}".jar /var/www/rectangles/all/green/rectangle_${env.MAJOR_VERSION}.${env.BUILD_NUMBER}".jar
      }
    }
  }
}
